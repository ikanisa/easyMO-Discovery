...
const BusinessCardWidget: React.FC<BusinessCardWidgetProps> = ({ 
  ...
  return (
    <div 
      className={`
        glass-panel rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 shadow-lg bg-white dark:bg-slate-900/40 transition-all duration-300
      ...
      <div className="p-4 flex flex-col h-full relative">
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="flex-1 pr-2">
            <h3 className="font-bold text-slate-900 dark:text-white leading-tight line-clamp-2 flex items-baseline gap-2">
              <span className="text-lg filter drop-shadow-lg">{categoryIcon}</span>
              <span>{biz.name}</span>
            </h3>
            <div className="text-[10px] font-bold text-blue-500 dark:text-blue-400 uppercase tracking-wide opacity-90 mt-1 pl-7">
              {biz.category}
            </div>
          </div>
          ...
          <div className={`
             ...
             ${isOpen ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 
               isClosed ? 'bg-red-500/10 border-red-500/20 text-red-600 dark:text-red-400' : 'bg-slate-100 dark:bg-slate-700/50 border-slate-200 dark:border-slate-600/50 text-slate-500 dark:text-slate-400'}
          `}>
        ...
        {/* Meta Row */}
        <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-300 mb-3 pl-1">
          <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-medium bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded">
        ...
        {biz.phoneNumber && (
          <div className="flex items-center gap-2 mb-4 pl-1 text-xs text-slate-600 dark:text-slate-300">
             <ICONS.Phone className="w-3 h-3 text-slate-400 dark:text-slate-500" />
             <span className="font-mono">{biz.phoneNumber}</span>
             <button 
               onClick={handleCopyPhone}
               className="ml-auto text-[10px] text-blue-500 dark:text-blue-400 hover:text-blue-400 dark:hover:text-blue-300 flex items-center gap-1 bg-blue-50 dark:bg-blue-500/10 px-2 py-1 rounded transition-colors"
             >
        ...
        `}>
           {biz.snippet && (
             <div className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-black/20 p-2 rounded-lg border-l-2 border-slate-300 dark:border-slate-600 italic">
               "{biz.snippet}"
             </div>
           )}
        </div>
        ...
        <div className="flex flex-col gap-2 mt-auto">
           ...
           <div className={`grid gap-2 ${biz.phoneNumber ? 'grid-cols-[1.5fr_1fr]' : 'grid-cols-1'}`}>
              <button 
                ...
                className="flex items-center justify-center gap-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-95 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-white/5 py-2.5 rounded-xl text-xs font-bold transition-all"
              >
                <ICONS.WhatsApp className="w-4 h-4 text-emerald-600 dark:text-emerald-500" />
                Message
              </button>
              ...
              {biz.phoneNumber && (
                <button 
                  onClick={handleCall}
                  className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold border transition-all active:scale-95 bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-white/10"
                >
