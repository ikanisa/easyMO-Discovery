          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center gap-3">
               <div className={`p-1 rounded-full transition-transform duration-300 ${isSelected ? 'scale-110' : 'group-hover:scale-110'}`}>
                  <Icon className={`w-8 h-8 ${isSelected ? 'text-white drop-shadow-md' : 'text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200'}`} />
               </div>
               <span className={`text-[10px] font-black tracking-widest uppercase ${isSelected ? 'text-white/90' : 'text-slate-600 dark:text-slate-500'}`}>
                 {v.label}
               </span>
            </div>
