...
const Layout: React.FC<LayoutProps> = ({ children, currentMode, onNavigate }) => {
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      {/* Thumb zone: Primary actions at bottom, safe area padding for iOS */}
      <nav 
        className="fixed bottom-0 left-0 w-full z-50 px-4 pb-4 pt-2 pointer-events-none" 
        style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}
        aria-label="Main navigation"
      >
        <div className="glass-panel rounded-2xl flex justify-around items-center min-h-16 shadow-2xl shadow-black/10 dark:shadow-black/50 pointer-events-auto bg-white/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-xl">
          <NavButton 
...
const NavButton: React.FC<NavButtonProps> = ({ active, onClick, icon: Icon, label }) => (
  <button 
    ...
    className={`
      ...
      focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:rounded-xl
      ${active ? 'text-blue-600 dark:text-blue-400 -translate-y-1' : 'text-slate-500 dark:text-slate-400'}
      touch-manipulation
    `.trim().replace(/\s+/g, ' ')}
    aria-label={label}
