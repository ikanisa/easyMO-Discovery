    ...
    try {
      ...
      if (useWorker) {
        try {
          ...
          let accumulatedText = '';
          let toolResults: any[] = [];
          let conversationId: string | undefined = initialSession.id;
          ...
          for await (const chunk of AgentService.chatStream(
            ...
            if (chunk.type === 'start') {
              ...
              if (chunk.agent_type && chunk.agent_type !== agentType) {
                console.log('Agent type changed:', chunk.agent_type);
              }
              ...
              // Tool result received - parse and store
              try {
                const result = typeof chunk.tool_result === 'string' 
                  ? JSON.parse(chunk.tool_result) 
                  : chunk.tool_result;
                toolResults.push(result);
                
                // Update message with tool result if it contains structured data
                if (result.success && (result.matches || result.intent_id || result.payment_id || result.vendor_id)) {
                  setMessages(prev => prev.map(msg => {
                    if (msg.id === aiMsgId) {
                      // Parse structured output for UI cards
                      if (result.matches && Array.isArray(result.matches)) {
                        // Mobility matches
                        return { ...msg, businessPayload: { matches: result.matches } };
                      } else if (result.intent_id) {
                        // Ride intent created
                        return { ...msg, location: result.pickup?.location };
              ...
              // Handle structured output from done chunk
              if (chunk.structured_output && Array.isArray(chunk.structured_output)) {
                // Process structured output for UI cards
                chunk.structured_output.forEach((output: any) => {
                  if (output.success) {
                    if (output.matches && Array.isArray(output.matches)) {
                      // Mobility matches - create business payload
                      setMessages(prev => prev.map(msg => {
                        if (msg.id === aiMsgId) {
                          return {
                            ...msg,
                            businessPayload: {
                              matches: output.matches.map((m: any) => ({
                                id: m.user_id || m.driver_id,
                                name: `Driver ${(m.user_id || m.driver_id).slice(0, 4)}`,
                                category: 'Driver',
                                distance: `${m.distance_km || (m.distance_m / 1000).toFixed(2)} km`,
                                confidence: 'High' as const,
                                phoneNumber: undefined,
                              })),
                            } as BusinessResultsPayload,
                          };
                        }
                        return msg;
                      }));
                    } else if (output.intent_id) {
                      // Ride intent created
                      setMessages(prev => prev.map(msg => {
                        if (msg.id === aiMsgId) {
                          return {
                            ...msg,
                            location: output.pickup?.location,
                          };
                        }
                        return msg;
                      }));
              ...
              setMessages(prev => prev.map(msg => 
                msg.id === aiMsgId 
                  ? { ...msg, text: accumulatedText || `Error: ${chunk.error || 'Unknown error'}`, isThinking: false }
                  : msg
              ));
            } else if (chunk.type === 'tool_result') {
              // Tool result received - parse and store
              const toolResultContent = chunk.tool_result || chunk.content;
              if (toolResultContent) {
                try {
                  const result = typeof toolResultContent === 'string' 
                    ...
                    setMessages(prev => prev.map(msg => {
                      if (msg.id === aiMsgId) {
                        ...
                        if (result.matches && Array.isArray(result.matches)) {
                          ...
                          return {
                            ...
                            businessPayload: {
                              matches: result.matches.map((m: any) => ({
                                id: m.user_id || m.driver_id || m.match_id,
                                name: `Driver ${(m.user_id || m.driver_id || '').slice(0, 4)}`,
                                category: 'Driver',
                                distance: `${m.distance_km || (m.distance_m ? (m.distance_m / 1000).toFixed(2) : '0')} km`,
                                confidence: 'High' as const,
                                snippet: m.eta_seconds ? `ETA: ${Math.ceil(m.eta_seconds / 60)} min` : undefined,
                              })),
                            } as BusinessResultsPayload,
                          };
                        } else if (result.intent_id) {
                          ...
                          return {
                            ...msg,
                            location: result.pickup?.location,
                          };
                        } else if (result.payment_id) {
                          // Payment receipt
                          return { ...msg };
                        } else if (result.vendor_id) {
                          // Vendor onboarded
                          return { ...msg };
                        }
                      }
              ...
              setMessages(prev => prev.map(msg => 
                msg.id === aiMsgId 
                  ? { ...msg, text: accumulatedText || msg.text, isThinking: false }
                  : msg
              ));
            }
          }
