...
const RadarView = ({ active, text }: { active: boolean, text: string }) => (
  <div className="flex flex-col items-center justify-center py-20 text-slate-500 gap-8 animate-in fade-in zoom-in duration-700">
     <div className="relative w-64 h-64 flex items-center justify-center">
        ...
        {/* Center Node */}
        <div className={`
           w-24 h-24 rounded-full flex items-center justify-center backdrop-blur-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] z-10 border transition-all duration-500
           ${active ? 'bg-gradient-to-br from-cyan-500/20 to-blue-600/20 border-cyan-500/50 shadow-cyan-500/20' : 'bg-slate-900/80 border-white/10'}
        `}>
           <ICONS.Map className={`w-10 h-10 transition-colors duration-500 ${active ? 'text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.8)]' : 'text-slate-600'}`} />
        </div>
     ...
     <div className="text-center space-y-2">
       <p className="text-sm font-bold tracking-[0.2em] uppercase text-slate-400 animate-pulse">
         {text}
       </p>
...
const Discovery: React.FC<DiscoveryProps> = ({ role, onStartChat, onBack }) => {
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#0f172a] text-white relative overflow-hidden">
      
      {/* Immersive Background Gradients */}
      <div className="absolute top-0 left-0 w-full h-[500px] bg-gradient-to-b from-indigo-900/40 via-slate-900/0 to-transparent pointer-events-none" />
      <div className="absolute top-[-100px] right-[-50px] w-96 h-96 bg-blue-500/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-[-100px] left-[-50px] w-96 h-96 bg-emerald-500/10 rounded-full blur-[100px] pointer-events-none" />

      {/* Floating Header */}
      <div className="relative z-30 px-6 pt-8 pb-4 flex justify-between items-center">
        <button 
           onClick={onBack}
           className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 backdrop-blur-md flex items-center justify-center transition-all active:scale-95 group"
        >
           <ICONS.ChevronDown className="w-5 h-5 rotate-90 text-slate-300 group-hover:text-white" />
        </button>
        ...
        {role === 'passenger' && (
           <div className="px-3 py-1 rounded-full bg-white/5 border border-white/10 backdrop-blur-md flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold tracking-widest uppercase text-slate-300">Live Map</span>
           </div>
        )}
      </div>
      ...
      <div className="flex-1 px-4 pb-32 relative z-10 overflow-y-auto no-scrollbar">
        ...
        {(role === 'driver' || role === 'vendor') && (
          <div className="mb-8 animate-in slide-in-from-bottom-4 duration-700">
            <div className={`
              ...
              ${isOnline ? 'bg-gradient-to-br from-emerald-400 via-teal-500 to-cyan-500 shadow-[0_0_40px_rgba(16,185,129,0.3)]' : 'bg-gradient-to-br from-slate-700 to-slate-800'}
            `}>
              <div className={`
                 ...
                 ${isOnline ? 'bg-slate-900/90' : 'bg-[#0f172a]'}
              `}>
                 {/* Online Radiance */}
                 {isOnline && <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-1 bg-emerald-400 blur-[20px]" />}

                 <div className="mb-4 relative">
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-all duration-500 ${isOnline ? 'bg-emerald-500/20 text-emerald-400 ring-2 ring-emerald-500/50 shadow-[0_0_30px_rgba(16,185,129,0.4)]' : 'bg-slate-800 text-slate-500 ring-1 ring-white/10'}`}>
                       {isOnline ? <ICONS.Broadcast className="w-8 h-8 animate-pulse" /> : <ICONS.Moon className="w-8 h-8" />}
                    </div>
                 </div>

                 <h2 className={`text-2xl font-black mb-1 ${isOnline ? 'text-white' : 'text-slate-400'}`}>
                    {isOnline ? 'You are Online' : 'You are Offline'}
                 </h2>
                 <p className="text-xs text-slate-400 font-medium mb-6 max-w-[200px]">
          ...
          <div className="mb-8 animate-in slide-in-from-bottom-4 duration-700 delay-100">
            <div className="flex justify-between items-center mb-4 px-2">
                ...
                <button 
                  ...
                  `}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${privacyMode ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'}`} />
        ...
        {locationError && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-center gap-4 animate-in fade-in backdrop-blur-md">
             ...
             <div className="flex-1">
                <h3 className="font-bold text-red-200 text-xs uppercase tracking-wide mb-1">Location Error</h3>
                <p className="text-xs text-red-200/60 leading-relaxed">{locationError}</p>
             </div>
             <button onClick={() => window.location.reload()} className="p-2 bg-red-500/20 rounded-lg text-red-200">
        ...
        <div className="flex-1 min-h-[300px]">
          <div className="flex justify-between items-end mb-4 px-2">
              ...
              {nearbyUsers.length > 0 && <span className="text-[10px] text-cyan-400 font-bold bg-cyan-500/10 px-2 py-1 rounded-lg border border-cyan-500/20">LIVE</span>}
          </div>
